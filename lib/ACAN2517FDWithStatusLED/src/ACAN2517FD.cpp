//----------------------------------------------------------------------------------------------------------------------
// A CAN driver for MCP2517FD, CANFD mode modified for LED control
// by Pierre Molinaro
...
//----------------------------------- Install interrupt, configure external interrupt
  ...
  //modified by K.Fukushima
  //ピン方向指定
    uint8_t dataDIR=0B00000000;
  writeRegister8(IOCON_REGISTER,dataDIR);
...
bool ACAN2517FD::tryToSend (const CANFDMessage & inMessage) {
  ...
  if (ok) {
    uint8_t dataIO=0B00000010;//LED ON
    writeRegister8(IOCON_REGISTER+1,dataIO);//LED ON
    mSPI.beginTransaction (mSPISettings) ;
      #ifdef ARDUINO_ARCH_ESP32
        taskDISABLE_INTERRUPTS () ;
      #else
      ...
      #endif
    mSPI.endTransaction () ;
    dataIO=0B00000000;//LED OFF
    writeRegister8(IOCON_REGISTER+1,dataIO);//LED OFF
  }
  return ok ;
}
